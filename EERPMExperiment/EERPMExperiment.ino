...
const int calibValLenAddress = 0;  //storing length of the calibration value so Arduino knows how much we need to read
const int calibValAddress = 1; // EEPROM memory address for storing the calibration value
...
//    }
    int reading = -22343;    // taking the average of ten scale readings
    int calibVal = 466; // dividing the average scale reading with the known weight to find the calibration value
    scale.set_scale(calibVal);             // calibrating the scale
    String calibValEEPROM = String(calibVal);
    int calibValLength = calibValEEPROM.length();
    
    EEPROM.update(calibValLenAddress, calibValLength);
    
    for (int i = 0; i < calibValLength; i++) // Waiting for the next ten seconds for the user to put the known weight on the scale
    {
      EEPROM.update(calibValAddress + i, calibValEEPROM.substring(i, i+1).toInt());
      Serial.println("Calibration address: " + String(calibValAddress + i));
      Serial.println("Calibration value: " + String(calibValEEPROM.substring(i, i+1)));
    }

    int calibValRedLength = EEPROM.read(calibValLenAddress);
    Serial.println("Length stored : " + String(EEPROM.read(calibValLenAddress)));
    String readValu = "";
     for (int i = 0; i < calibValRedLength; i++) // Waiting for the next ten seconds for the user to put the known weight on the scale
    {
      readValu += EEPROM.read(calibValAddress + i);
    }

    Serial.println("Read calibration : " + String(readValu));
